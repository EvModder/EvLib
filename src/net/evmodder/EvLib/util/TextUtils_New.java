...
public class TextUtils_New{
	...
	private static final char[] FORMAT_CHARS = new char[]{'k', 'l', 'm', 'n', 'o', 'r'};
	private static final String[] FORMATS = new String[FORMAT_CHARS.length];
	static{for(int i=0; i<FORMATS.length; ++i) FORMATS[i] = new String(new char[]{COLOR_CHAR, FORMAT_CHARS[i]});}
	...
	public static Character getSimpleColorByName(String name){
	...
	public static String translateAlternateColorCodes(char altColorChar, String textToTranslate, String resetColor){
		return translateAlternateColorCodes(altColorChar, textToTranslate).replace(FORMATS[5], resetColor);
	}
...
//	private static String stripColor(final String input){return input == null ? null : STRIP_COLOR_PATTERN.matcher(input).replaceAll("");}
	...
	public static String getLeadingColorAndFormats(String str, char altColorChar){
		//TODO: if bold whitespace at start, folowed by arbitrary colored+formated text, should it just return '§l'?
		// Similar to minimizeColorCodes()
		String colorPat = "(?:"+altColorChar+"x(?:"+altColorChar+"[0-9a-fA-F]){6})|(?:"+altColorChar+"[0-9a-fA-FrR])";
		Pattern colorsAndSpacesPattern = Pattern.compile(
				"^((?:\\s|§.)*?)" // Match the fluff before the last color
				+ "("+colorPat+")(?!(?:\\s|"+altColorChar+".)*(?:"+colorPat+"))" // Match last color (before first text)
				+ "((?:(?:\\s|"+altColorChar+"[^k-oK-O])*(?:"+altColorChar+"[k-oK-O]))*)" // Glob with all formats after last color (before first text)
				+ ".*", Pattern.DOTALL); // Rest of str
		Matcher matcher1 = colorsAndSpacesPattern.matcher(str);
		String color = "", formats = "";
		if(matcher1.find()){
			color = matcher1.group(2);
			formats = matcher1.group(3).replaceAll("\\s|"+altColorChar, "");
		}
		else{
			Pattern formatsAndSpacesPattern = Pattern.compile("^((?:"+altColorChar+".|\\s)*).*", Pattern.DOTALL); // Rest of str
			Matcher matcher2 = formatsAndSpacesPattern.matcher(str);
			if(matcher2.find()) formats = matcher2.group(1).replaceAll("\\s|"+altColorChar, "");
		}
		if(formats.isEmpty()) return color;
		return color
				+ (formats.indexOf('k') != -1 ? FORMATS[0] : "")
				+ (formats.indexOf('l') != -1 ? FORMATS[1] : "")
//				+ (formats.indexOf('l') != -1 && str.matches("(?s)(?:\\n|"+altColorChar+".)*§l.*") ? FORMATS[1] : "")
				+ (formats.indexOf('m') != -1 ? FORMATS[2] : "")
				+ (formats.indexOf('n') != -1 ? FORMATS[3] : "")
				+ (formats.indexOf('o') != -1 ? FORMATS[4] : "");
	}
...
/*	public static String getCurrentColorAndFormats(String str){
	...
	public static String formatTime(long millis, boolean show0s, String timePrefix, String unitPrefix, String sep, int sigUnits, long[] scale, char[] units){
		timePrefix = timePrefix == null ? "" : timePrefix;
		timePrefix = unitPrefix == null ? "" : unitPrefix;
...
//				return 6;
	...
	public static double strLenExact(String str, boolean mono){
		...
		for(char ch : str.toCharArray()){
			...
			if(ch == COLOR_CHAR){colorPick = true; continue;}
			len += pxLen(ch);
